 ...
 */
export async function registerENSName(ensName: string, walletAddress: string): Promise<{
  success: boolean;
  message: string;
  transactionHash?: string;
}> {
  const {ENS_CONTRACT_ADDRESS, RPC_PROVIDER_URL, PRIVATE_KEY} = Bun.env;
  ...
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'An unknown error occurred during ENS registration.'
    };
  }
}